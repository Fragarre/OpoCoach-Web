...
export default function EmpleoProcesoPage(){
  ...
  const [me,setMe]=useState<Me|null>(null); const [proceso,setProceso]=useState<Proceso|null>(null); const [publicaciones,setPublicaciones]=useState<Publicacion[]>([]); const [cambios,setCambios]=useState<Cambio[]>([]); const [cargando,setCargando]=useState(true); const [error,setError]=useState("");
  useEffect(()=>{if(!id)return;let activo=true;async function cargar(){setCargando(true);setError("");try{const{data,error:authError}=await supabase.auth.getSession();if(authError)throw new Error(authError.message);const token=data.session?.access_token;if(!token)throw new Error("Se requiere autenticación.");const[m,encontrado,p,c]=await Promise.all([getJson<Me>("me",token),getJson<Proceso>(`procesos/${id}`,token),getJson<Publicacion[]>(`procesos/${id}/publicaciones`,token),getJson<Cambio[]>(`procesos/${id}/cambios`,token)]);if(!activo)return;if(!encontrado.es_oportunidad)throw new Error("La convocatoria no está disponible.");setMe(m);setProceso(encontrado);setPublicaciones(p);setCambios(c)}catch(e){if(activo)setError(e instanceof Error?e.message:String(e))}finally{if(activo)setCargando(false)}}void cargar();return()=>{activo=false}},[id,supabase]);
