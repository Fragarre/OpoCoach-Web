...
export default function EmpleoProcesoPage(){
  ...
  useEffect(()=>{if(!id)return;let activo=true;async function cargar(){setCargando(true);setError("");try{const{data,error:authError}=await supabase.auth.getSession();if(authError)throw new Error(authError.message);const token=data.session?.access_token;if(!token)throw new Error("Se requiere autenticación.");const[m,encontrado,p,c,o]=await Promise.all([getJson<Me>("me",token),getJson<Proceso>(`procesos/${id}`,token),getJson<Publicacion[]>(`procesos/${id}/publicaciones`,token),getJson<Cambio[]>(`procesos/${id}/cambios`,token),getJson<Organismo[]>("organismos",token)]);if(!activo)return;if(!encontrado.es_oportunidad)throw new Error("La convocatoria no está disponible.");setMe(m);setProceso(encontrado);setOrganismo(o.find(x=>x.id===encontrado.organismo_id)||null);setPublicaciones(p);setCambios(c)}catch(e){if(activo)setError(e instanceof Error?e.message:String(e))}finally{if(activo)setCargando(false)}}void cargar();return()=>{activo=false}},[id,supabase]);
