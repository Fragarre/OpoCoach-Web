...
export default function NovedadesAviso(){
 ...
 useEffect(()=>{if(pathname!=="/empleo"){setCargando(false);return;}let activo=true;(async()=>{try{const {data,error:e}=await supabase.auth.getSession();if(e||!data.session){if(activo)setCargando(false);return;}const token=data.session.access_token;const headers={Authorization:`Bearer ${token}`};const[nr,er,pr]=await Promise.all([fetch("/api/empleo/seguimiento/cambios",{cache:"no-store",headers}),fetch("/api/empleo/seguimiento/estado",{cache:"no-store",headers}),fetch("/api/empleo/procesos",{cache:"no-store",headers})]);if(!nr.ok||!er.ok||!pr.ok)throw new Error("No se pudo consultar el seguimiento");const novedades=(await nr.json()) as Novedad[];const estado=(await er.json()) as Estado;const procesos=(await pr.json()) as Proceso[];if(!activo)return;if(!estado.ultima_novedad_vista_at){setCantidad(0);setCargando(false);return;}const visibles=new Set(procesos.map(p=>p.id));const visto=new Date(estado.ultima_novedad_vista_at).getTime();const total=novedades.filter(n=>visibles.has(n.proceso_id)&&(n.novedad_tipo==="PUBLICACION"?esPublicacionUtil(n):esCambioUtil(n))&&(n.detectado_at?new Date(n.detectado_at).getTime():0)>visto).length;setCantidad(total);}catch{if(activo)setError(true)}finally{if(activo)setCargando(false)}})();return()=>{activo=false};},[pathname,supabase]);
