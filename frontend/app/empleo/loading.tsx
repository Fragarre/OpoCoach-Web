    ...
    <main style={styles.main}>
      <div style={styles.overlay} role="status" aria-live="polite" aria-busy="true">
        <div style={styles.box}>
          ...
          Cargando Empleo público…
        </div>
      </div>
      <style>{`@keyframes empleo-loading-spin { to { transform: rotate(360deg); } }`}</style>
    </main>
  );
}
...
const styles: Record<string, React.CSSProperties> = {
  ...
  spinner: { width: 18, height: 18, border: "2px solid #d9dee8", borderTopColor: "#172033", borderRadius: "50%", display: "inline-block", animation: "empleo-loading-spin .8s linear infinite" },
};
